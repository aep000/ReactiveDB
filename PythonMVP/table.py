...
class PersistentTable:
    ...
    def __str__(self):
        return "====TABLE PRINT OUT==== Name: {name}, Type:{tableType}\nvalues:\n{values}".format(name=self.name, tableType=self.table_type, values=[cbor2.loads(self.storage_manager.read_data(e.value)) for e in self.index.get_all()]) 

    def add_data(self, key, value):
        block = self.storage_manager.write_data(cbor2.dumps(value))
    ...
    def get_data(self, key):
        ...
        if(len(entry) == 0):
            ...
            return cbor2.loads(self.storage_manager.read_data(entry[0].value))
