...
impl Comparison {
    fn evaluate(&self, left: EntryValue, right:EntryValue) -> Result<EntryValue, String>{
        match (left, right) {
            (EntryValue::Integer(x), EntryValue::Integer(y)) => Ok(self.does_ordering_pass(x.cmp(&y))),
            ...
            (EntryValue::Bool(x), EntryValue::Bool(y)) => {
                match self {
                    ...
                    _ => Err(format!("Unable to do operation {:?} on {:?} and {:?}", self, x, y))
                }
            },
            (left, right) => Err(format!("Unable to do operation {:?} on {:?} and {:?}", self, left, right))
        }
    }
...
impl Operation {
    fn evaluate(&self, left: EntryValue, right:EntryValue) -> Result<EntryValue, String>{
        match (left, right) {
            ...
            (EntryValue::Str(x), EntryValue::Str(y)) => {
                match self {
                    Operation::Add => {
