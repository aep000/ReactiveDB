...
impl Hook for ListenerHook {
    fn execute(&mut self, event: Event, proposed_edits:Option<Vec<DBEdit>>, commited_edits: Option<Vec<CommitedEdit>>, _db: &mut Database, workspace: Workspace) -> Option<Vec<DBEdit>>{
        let default = vec![];
